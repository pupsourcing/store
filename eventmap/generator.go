...
`)
	...
	for eventType, versions := range eventsByType {
		...
		for _, event := range versions {
			...
			fmt.Fprintf(&sb, "\t\t\tvar e %s.%s\n", event.PackageName, event.Name)
			sb.WriteString("\t\t\tif err := json.Unmarshal(pe.Payload, &e); err != nil {\n")
			fmt.Fprintf(&sb, "\t\t\t\treturn nil, fmt.Errorf(\"failed to unmarshal %s v%d: %%w\", err)\n",
		...
		sb.WriteString("\t\tdefault:\n")
		fmt.Fprintf(&sb, "\t\t\treturn nil, fmt.Errorf(\"unknown version %%d for event type %s\", pe.EventVersion)\n",
			eventType)
		sb.WriteString("\t\t}\n")
	}
...
func (g *Generator) generateTypeHelpers() string {
	...
	for _, event := range g.events {
		// ToXXXVN function
		fmt.Fprintf(&sb, `// To%sV%d converts a domain event to a store.Event.
...
`,
		...
		// FromXXXVN function
		fmt.Fprintf(&sb, `// From%sV%d converts a persisted event to a domain event.
...
func (g *Generator) generateTestCode() string {
	...
	// File header
	fmt.Fprintf(&sb, `// Code generated by eventmap-gen. DO NOT EDIT.
...
func (g *Generator) generateTestToESEvents() string {
	...
	fmt.Fprintf(&sb, `// TestToESEvents tests the ToESEvents function with generics.
...
func (g *Generator) generateTestFromESEvents() string {
	...
	fmt.Fprintf(&sb, `// TestFromESEvents tests the FromESEvents function.
...
func (g *Generator) generateTestOptions() string {
	...
	fmt.Fprintf(&sb, `// TestOptions tests the Options pattern.
...
func (g *Generator) generateTestTypeHelpers() string {
	...
	fmt.Fprintf(&sb, `// TestTypeHelpers tests type-specific helper functions.
